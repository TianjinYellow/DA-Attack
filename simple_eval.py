...
def load_images(input_dir, batch_shape):
    ...
    for filepath in tf.gfile.Glob(os.path.join(input_dir, '*.JPEG')):
        with tf.gfile.Open(filepath, 'rb') as f:
            image = (imread(f).astype(np.float) / 255.0)[:,:,:3]
...
if __name__ == '__main__':
    ...
    input_dir = './new_exp/IncResV2_outputs_steps_22_new'
    #input_dir="./FFT/outputs_DIM_perturb_14"
    print(input_dir)
    #print("evalute:", FLAGS.folder,"  Folder")
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
            ...
            for i in range(len(model_name)):
                print("Attack Success Rate for {0} : {1:.1f}%".format(model_name[i], success_count[i] / 970. * 100))
